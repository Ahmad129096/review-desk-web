...
import { EmptyState, Metric, ReviewItem, StatusPill } from "../components/SharedComponents";
import { useBusinesses, useReviews, useUpdateReview, useDeleteReview, useImportReviews, api } from "../api-query";
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                <div>
                  <div style={{ fontSize: '14px', opacity: 0.9, marginBottom: '4px' }}>Loading...</div>
                  <div style={{ fontSize: '32px', fontWeight: '800', lineHeight: '1', background: 'rgba(255,255,255,0.2)', borderRadius: '4px', width: '60px', height: '32px' }}></div>
                </div>
                <div style={{
                  width: '48px',
                  height: '48px',
                  ...
                  <div style={{ width: '24px', height: '24px', background: 'rgba(255,255,255,0.3)', borderRadius: '4px' }}></div>
                </div>
              </div>
              <div style={{ fontSize: '14px', opacity: 0.8 }}>
                <div style={{ width: '80px', height: '16px', background: 'rgba(255,255,255,0.2)', borderRadius: '4px' }}></div>
              </div>
            </div>
          ))}
        </section>
...
function BusinessSetup({ onCreated }: { onCreated: (business: Business) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      const response = await api.createBusiness({ name, industry, defaultTone, website: "", phone: "", address: "" });
      onCreated((response.data as any).data.business);
    } catch (err) {
  ...
  return (
    <section className="panel narrow">
      ...
      <form className="stack" onSubmit={submit}>
        ...
        <label>
          Reply tone
          <select value={defaultTone} onChange={(event) => setDefaultTone(event.target.value as Business["defaultTone"])}>
...
function ReviewsContent({
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      const response = await api.createReview({ businessId: business.id, reviewerName, rating, content });
  ...
  async function importCsv(event: FormEvent) {
    ...
    setError("");
    try {
      const response = await api.importReviews({ businessId: business.id, csv });
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {filteredReviews.map((review) => (
            <ReviewItem key={review.id} review={review} onStatusChange={onStatusChange} onDeleted={onDeleted} />
          ))}
...
function GoogleIntegration({ business, onSynced }: { business: Business; onSynced: (reviews: Review[]) => void }) {
  ...
  async function loadLocations() {
    ...
    try {
      ...
      setMessage((response.data as any).data.locations.length ? "Select the matching Google Business location." : "No Google locations were returned.");
    } catch (err) {
  ...
  async function syncReviews() {
    ...
    setError("");
    try {
      const response = await api.googleSyncReviews({ businessId: business.id, autoReply, autoReplyMinRating });
      onSynced((response.data as any).data.reviews);
      setMessage(`Synced ${(response.data as any).data.importedCount} Google reviews. Published ${(response.data as any).data.publishedReplies} replies.`);
    } catch (err) {
  ...
  return (
    <section style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: '500', color: '#344054', cursor: 'pointer' }}>
          <input
            type="checkbox"
            checked={autoReply}
        ...
        <div>
          ...
          <select
            ...
            onChange={(event) => setAutoReplyMinRating(Number(event.target.value))}
            style={{
              width: '100%',
              padding: '12px 16px',
              border: '1px solid #d8e1ec',
